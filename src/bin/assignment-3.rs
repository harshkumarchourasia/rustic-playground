...
Write a program which will first declare two points p1 and p2 using tuples and 
will Initialize the tuple p1 from the values of (4.0, 3.0) and the tuple p2 from the values of (5.0, 4.5).
...
fn main(){
    ...
    let dist = (f64::powf(p1.0 - p2.0, 2.0) + f64::powf(p1.1 - p2.1, 2.0)).sqrt();
    println!("{:?}", dist);
}